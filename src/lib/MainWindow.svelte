<script lang="ts">
  ...
  async function loadSongs(){
    songs = await invoke("load_dir", {musicDir: localStorage.getItem("musicDir"), options: options}) 
...
<div>
  {#if ready}
  <div transition:fly={{ delay: 500, duration: 700,  x: -400}} class="w-5/12 h-full fixed bg-white dark:bg-gray-600 pt-8 pb-8 shadow-xl ring-1 ring-gray-900/5 sm:mx-auto max-w-lg rounded-r-3xl px-5 left-0 flex border-black">
    <div class="h-2 fixed">
      ...
      <dialog transition:fade={{}} class="rounded-lg h-1/2 aspect-square bg-white dark:bg-gray-800" id="settings" bind:this={settingsDialog} on:click={closeSettings}>
        ...
        <div class="mb-4 border-white rounded-lg border-white">
          <label class="text-white"><strong>Search Directories Recursively</strong></label>
          <input type="checkbox" class="form-checkbox mx-2 h-4 w-4" bind:checked={options.recursive}> 
        </div>
        </div>
        <button class="rounded-md self-start" on:click={localStorage.setItem("options", JSON.stringify(options))}>Save</button>
      </dialog>
  ...
  <div transition:fly={{ delay: 100, duration: 1000, y: 1000}} class="fixed justify-end xl:w-8/12 w-7/12 h-full right-0 overflow-x-auto">
    <table class="table-fixed">
      ...
      <tbody class="overflow-y-auto">
        {#each songs as song}
        <tr in:fade={{delay: 100, duration: 500}} class="transition ease-in-out hover:dark:bg-gray-800 duration-500 rounded-l-3xl max-h-4 {activeElement === song.file_name ? 'active' : ''}" id={song.file_name} on:click={() => {changeSong(song); changeActiveElement(song.file_name)}}>
