...
pub enum LoadAudioDirValue {
    Finshed,
    AudioFile(Result<AudioFile>),
}

#[tauri::command(rename_all = "camelCase")]
pub fn load_audio_file(path: PathBuf) -> Result<AudioFile> {
...
#[tauri::command(rename_all = "camelCase")]
pub fn load_audio_dir(directory: PathBuf, on_file_processed: Channel<LoadAudioDirValue>) {
...
pub fn get_audio_cover(path: &Path) -> Result<Option<String>> {
    Ok(Tag::new()
        .read_from_path(path)?
        .album_cover()
        .and_then(|cover| {
...
#[tauri::command(rename_all = "camelCase", async)]
pub fn save_audio_tags(tags: AudioFile, remove_cover: bool) -> Result<()> {
