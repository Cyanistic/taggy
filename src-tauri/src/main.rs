...
fn load_dir(music_dir: String, recursive: bool) -> SerResult<Vec<HashMap<String, String>>> {
    ...
    for i in files {
        let tag;
        let file_name = Path::new(&i.clone())
            .file_name()
            .unwrap_or(&OsStr::new(""))
            .to_string_lossy()
            .to_string();
        ...
        tag_map.insert(
            ...
            format!(
                "{}/{}",
                tag.track().0.unwrap_or(0),
                tag.track().1.unwrap_or(0)
            )
            .to_string(),
        );
        tag_map.insert(
            "album_title".to_string(),
            tag.album_title().unwrap_or("").to_string(),
        );
        tag_map.insert(
            "album_artist".to_string(),
            tag.album_artist().unwrap_or("").to_string(),
        );
        tag_map.insert(
            "disc_number".to_string(),
            format!(
                "{}/{}",
                tag.disc().0.unwrap_or(0),
                tag.disc().1.unwrap_or(0)
            )
            .to_string(),
        );
        tag_map.insert(
            "cover_data".to_string(),
            BASE64_STANDARD
                .encode(
                    tag.album_cover()
                        .unwrap_or(Picture {
    ...
    tags.sort_unstable_by(|a, b| {
        a[&"file_name".to_string()]
            .partial_cmp(&b[&"file_name".to_string()])
            .unwrap()
    });
    Ok(tags)
}
        ...
        match key {
            ...
            "cover_data" => {
                ...
                tag.set_album_cover(Picture {
                    data: BASE64_STANDARD.decode(val)?.as_slice(),
                    mime_type: mime,
                });
            }
            "disc_number" => {
...
#[tauri::command]
fn search<'a>(
    songs: Vec<HashMap<&'a str, &'a str>>,
    pattern: &'a str,
) -> Vec<HashMap<&'a str, &'a str>> {
    return songs
        .into_iter()
        .filter(|x| {
            x[&"file_name"].to_lowercase().contains(pattern)
                || x[&"title"].to_lowercase().contains(pattern)
                || x[&"artist"].to_lowercase().contains(pattern)
                || x[&"album_artist"].to_lowercase().contains(pattern)
                || x[&"album_title"].to_lowercase().contains(pattern)
        })
        .collect();
}
